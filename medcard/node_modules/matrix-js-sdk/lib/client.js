...
exports.RoomVersionStability = exports.PendingEventOrdering = exports.MatrixClient = exports.M_AUTHENTICATION = exports.ClientEvent = exports.CRYPTO_ENABLED = void 0;
...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
  ...
  async sendReceipt(event, receiptType, body, callback) {
  ...
  paginateEventTimeline(eventTimeline, opts) {
    ...
    if (isNotifTimeline) {
      ...
      promise = this.createMessagesRequest(eventTimeline.getRoomId(), token, opts.limit, dir, eventTimeline.getFilter()).then(res => {
        ...
        const atEnd = res.end === undefined || res.end === res.start; // if we've hit the end of the timeline, we need to stop trying to
        // paginate. We need to keep the 'forwards' token though, to make sure
        // we can recover from gappy syncs.
  ...
  requestLoginToken(auth) {
    ...
    return this.http.authedRequest(undefined, // no callback support
    _httpApi.Method.Post, "/org.matrix.msc3882/login/token", undefined, // no query params
    body, {
      prefix: _httpApi.PREFIX_UNSTABLE
    });
  }
  ...
  * @return {Object} the response, with chunk, prev_batch and, next_batch.
  ...
  async addThreePidOnly(data) {
    ...
    return this.http.authedRequest(undefined, _httpApi.Method.Post, path, undefined, data, {
      prefix
    });
  }
  /**
  ...
  async getPushers(callback) {
    const response = await this.http.authedRequest(callback, _httpApi.Method.Get, "/pushers"); // Migration path for clients that connect to a homeserver that does not support
  ...
  setPusher(pusher, callback) {
    ...
    return this.http.authedRequest(callback, _httpApi.Method.Post, path, undefined, pusher);
  }
  /**
